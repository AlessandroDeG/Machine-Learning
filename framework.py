...
class Softmax(Layer):
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        #result=exps / np.sum(exps,axis=1)[:,None]
    ...
    def backward(self, grad_in: np.ndarray) -> Layer.BackwardResult:
        ...
        d_x = softmax * (grad_in-(grad_in*softmax).sum(axis=1)[:,None])
        
        #SM = softmax.reshape((-1,1))
        #jac = np.diagflat(softmax) - np.dot(SM, SM.T)

        #x_grad = np.zeros(softmax.shape)
        #for i in range(softmax.shape[0]):
        #    for j in range(softmax.shape[1]):
        #        x_grad[i, j] = (softmax[i, j] * (1 - softmax[i, j]))
        #d_x=x_grad
...
class CrossEntropy(Loss):
    def forward(self, prediction: np.ndarray, target: np.ndarray) -> float:
        ...
        #Y = Y.clip(min=1e-8,max=None)
        #mean_ce= (np.where(T==1,-np.log(Y), 0)).sum(axis=1)/n

        #mean_ce = np.sum(T*(-np.log(Y)))/n
        #epsilon=1e-12
        ...
        mean_ce = -np.sum(target*np.log(prediction))/N
    ...
    def backward(self) -> np.ndarray:
        ...
        #y= y.clip(min=1e-8,max=None)
        #d_prediction=np.where(T==1,-1/y, 0)
...
def train_one_step(model: Layer, loss: Loss, learning_rate: float, input: np.ndarray, target: np.ndarray) -> float:
    ...
    #for key in model.var.keys():
        #print("modelvar: %s" % key)
        #print(model.var[key].shape)
        #print("variablegrad")
        #print(variable_gradients[key].shape)

        #model.var[key] = np.subtract(model.var[key], learning_rate*variable_gradients[key]) 
...
def gradient_check():
    ...
    for key, value in NN.var.items():
        ...
        for index in range(variable.shape[0]):
            ...
            left=loss.forward(NN.forward(X), T)
            variable[index]=var_backup+(eps)
            right=loss.forward(NN.forward(X), T)
            
        
            numeric_grad = (right-left)/(2*eps)
