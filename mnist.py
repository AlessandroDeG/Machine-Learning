...
def verify(images: np.ndarray, targets: np.ndarray) -> Tuple[int, int]:
    ## Implement
    y = model.forward(images) #prediction probabilities
    #print("PREDICTIONS")
    #print(y)
    i = np.argmax(y, axis=1) #index
    ...
    num_ok = np.sum(i == targets) #index == label?
    #print("PREDICTIONS NUM_OK")
    #print(num_ok)
    total_num = len(i)
    ## End
...
for epoch in range(1000):
    ...
    for i in range(0, len(train_indices), batch_size):
        ...
        sum += lib.train_one_step(model, loss, learning_rate, images, labels) * len(labels)
        #loss_value=lib.train_one_step(model, loss, learning_rate, images, labels) * len(labels)

    loss_value = sum / len(train_indices)
    ...
    if validation_accuracy > best_validation_accuracy:
        ...
        if epoch - best_epoch > 10:
            print("\t early stop: %d iterations." % (epoch - best_epoch))
            break
